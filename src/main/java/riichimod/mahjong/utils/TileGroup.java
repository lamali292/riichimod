...
public class TileGroup implements Holdable
{
    ...
    public MahjongTileKind getFirstTileKind() {
        return MahjongTileKind.getKindFromIndex(tileIndices.get(0));
    }
    ...
    {
        int highestIndex = Math.max(tileIndices.get(0), tileIndices.get(1));
        int lowestIndex = Math.min(tileIndices.get(0), tileIndices.get(1));
    ...
    {
        StringBuilder toReturn = new StringBuilder("TileGroup : ");
        for (Integer index : tileIndices)
        {
            toReturn.append(MahjongTileKind.getKindFromIndex(index).name()).append(" ");
        }
    ...
    public String toMPSZNotation()
    {
        ...
        for (Integer index : tileIndices)
        {
            toReturn.append(MahjongTileKind.getKindFromIndex(index).getMPSZNumber());
        }
        toReturn.append(MahjongTileKind.getKindFromIndex(tileIndices.get(0)).getMPSZFamily());

        return toReturn.toString();
    }

    public boolean isHonor() {
        return getTileKinds().stream().allMatch(MahjongTileKind::isHonour);
    }

    public boolean isNumeral() {
        return getTileKinds().stream().allMatch(MahjongTileKind::isNumeral);
    }
}
